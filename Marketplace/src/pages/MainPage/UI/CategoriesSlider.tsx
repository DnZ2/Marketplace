            ...
            <Swiper
                ...
                {data?.map((item)=>
                    <SwiperSlide key={item}>
                        <NavLink to={`/products?category=${item}`} className="flex flex-col justify-center items-center gap-4 border border-[#7D8184] rounded py-6 hover:bg-[#db4444] [&>svg>g>path]:hover:stroke-white [&>svg>g>line]:hover:stroke-white [&>p]:hover:text-white hover:border-none">
                            <Phones />
                            <p>{item}</p>
                        </NavLink>
                    </SwiperSlide>
