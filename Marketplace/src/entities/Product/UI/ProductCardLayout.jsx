...
const ProductCardLayout = ({image=NoPhoto, controls, data}) => {
	...
	const {id, title, price, discount, rating} = data
  return (
	<div className="flex flex-col gap-4">
		<div className="flex justify-center items-center h-[250px] max-w-[300px] rounded-lg overflow-hidden bg-[#f0f0f0] relative [&>button]:hover:bottom-0">
			<img className="h-[60%] object-contain" src={image} alt="productImage" />
			<div className="absolute left-3 top-3 flex flex-col gap-1">
				{discount>0? <Sale>{discount+"%"}</Sale> : null}
				<Sale bg="#18864B">NEW</Sale>
			</div>
			<div className="absolute right-3 top-3 flex flex-col gap-2">{
				controls==="delete"
				? <DeleteButton onClick={()=>dispatch(removeProductFromFavourite(data))}/>
				: <FavouriteButton onClick={handleToggleFavouriteProduct} data={data} />
			}
			</div>
		{
			...
			:
			<button className="rounded-b-lg flex justify-center items-center gap-2 w-full bg-black text-[#f0f0f0] p-2 transition-all absolute bottom-[-40px]"
		...
		<div className="flex flex-col gap-1">
			<NavLink to={`products/${id}`} className='text-base'>{title}</NavLink>
			<div className='text-[#DB4444]'>
				${discount>0 ? (price - price*discount/100).toFixed(1) : price}
				<span className="pl-3 line-through text-[#7D8184]">
